...
const PostList = ({posts, removePost}) => {
    ...
    return (
        <ul>
            {posts.map(post => (
                <li className="card mb-3" key={post.id}>
                    <div className="card-header">
                        Post #{post.id}
                    </div>
                    <div className="card-body">
                        <div className="d-flex justify-content-between align-items-center flex-wrap card-content">
                            ...
                            <div className="col-sm-2 d-flex flex-column">
                                <button className="btn btn-secondary align-self-center mt-2" onClick={() => {
                                    router.push(`/${post.id}`)
                                }}>Open post
                                </button>
